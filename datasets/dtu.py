...
def create_spheric_poses(cameras, n_steps=120, reverse_up=False):
...
class DTUDatasetBase():
    def setup(self, config, split):
        ...
        if 'img_wh' in self.config:
            ...
            # assert round(W / w * h) == H # for anisotropic scaling
        ...
        for i in range(n_images):
            ...
            fx, fy, cx, cy = K[0,0] * self.factorX, K[1,1] * self.factorY, K[0,2] * self.factorX, K[1,2] * self.factorY
        ...
        if self.split == 'test':
            self.all_c2w = create_spheric_poses(self.all_c2w[:49,:,3] * self.config.scale_test_traj, n_steps=self.config.n_test_traj_steps, reverse_up=self.config.reverse_up)
