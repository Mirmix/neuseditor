...
class VanillaMLP(nn.Module):
    def __init__(self, dim_in, dim_out, config):
        ...
        self.edit = config.get('edit', False)
        self.fusion_edit = config.get('fusion_edit', False)
    ...
    def make_linear(self, dim_in, dim_out, is_first, is_last):
        ...
        if self.sphere_init:
            if is_last:
                if self.edit:
                    # initialize to zero
                    # torch.nn.init.constant_(layer.bias, 4.0)
                ...
                #     # initialize to identity
                #     torch.nn.init.constant_(layer.bias, 0.0)
                #     torch.nn.init.eye_(layer.weight[:,:13])                
            else:
                torch.nn.init.constant_(layer.bias, 0.0)
                torch.nn.init.normal_(layer.weight, 0.0, math.sqrt(2) / math.sqrt(dim_out))
        else:
            if is_last and self.edit:
                torch.nn.init.constant_(layer.bias, 0.0)
                torch.nn.init.constant_(layer.weight, 0.0)
            elif is_last and self.fusion_edit:
                torch.nn.init.constant_(layer.bias, 0.0)
                torch.nn.init.constant_(layer.weight, 0.0)
            else:
                torch.nn.init.constant_(layer.bias, 0.0)
