...
class VolumeDensity(BaseImplicitGeometry):
    ...
    def forward(self, points):
        ...
        if self.config.mlp_network_config.edit:
            density = density - torch.exp(torch.tensor(-1.))
...
@models.register('volume-sdf-edit')
class VolumeSDFEdit(BaseImplicitGeometry):
    def setup(self):
        self.n_output_dims = self.config.feature_dim
        ...
        network_edit= get_mlp(self.n_output_dims*2+35, self.n_output_dims, self.config.mlp_network_config_edit)
        self.encoding, self.network, self.network_edit = encoding, network, network_edit
    ...
    def forward(self, points, feature, with_grad=True, with_feature=True, with_laplace=False, feature_grad=None):
        with torch.inference_mode(torch.is_inference_mode_enabled() and not (with_grad and self.grad_type == 'analytic')):
            with torch.set_grad_enabled(self.training or (with_grad and self.grad_type == 'analytic')):
                ...
                # append the sdf value to the feature to out
                out = torch.cat([feature, out], dim=-1) # Feature from edit and feature from identity network
                out = torch.cat([out,self.encoding(points.view(-1, 3))], dim=-1) # Condition the network on the position encoding   
                out = (self.network_edit(out)).view(*points.shape[:-1], self.n_output_dims).float()
                out = out + feature
                ...
                if with_grad:
                    if self.grad_type == 'analytic':
                        ...
                        out = self.network(self.encoding(points_d.view(-1, 3))).view(-1, self.n_output_dims).float()
                        out = torch.cat([feature_grad, out], dim=-1) # Feature from edit and feature from identity network
                        out = torch.cat([out,self.encoding(points_d.view(-1, 3))], dim=-1) # Condition the network on the position encoding   
                        points_d_sdf = ((self.network_edit(out)).view(-1, self.n_output_dims).float() + feature_grad)[...,0].view(-1,6)
...
class VolumeSDF(BaseImplicitGeometry):
    ...
    def forward(self, points, with_grad=True, with_feature=True, with_laplace=False, with_feature_grad=False):
        with torch.inference_mode(torch.is_inference_mode_enabled() and not (with_grad and self.grad_type == 'analytic')):
            with torch.set_grad_enabled(self.training or (with_grad and self.grad_type == 'analytic')):
                if with_grad and self.grad_type == 'analytic':
                    if not self.training:
                ...
                if with_grad:
                    if self.grad_type == 'analytic':
                        ...
                        points_d_ = (points_[...,None,:] + offsets).clamp(-self.radius, self.radius)
                        points_d = scale_anything(points_d_, (-self.radius, self.radius), (0, 1))
                        feature_d = self.network(self.encoding(points_d.view(-1, 3))).view(-1, self.n_output_dims).float()
